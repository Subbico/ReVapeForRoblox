...
local function finishLoading()
		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript =[[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/Subbico/ReVapeForRoblox/'..readfile('ReVape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
				end
			]]
			if shared.VapeDeveloper then
	...
	if not shared.vapereload then
		...
		if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
			vape:CreateNotification('Onyx', "Initialized as " .. getgenv().username .. " with role "..shared.vape.role, 3)
			task.wait(2.75)
...
if not shared.VapeIndependent then
	...
	if isfile('ReVape/games/'..game.PlaceId..'.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/Subbico/ReVapeForRoblox/'..readfile('ReVape/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
			end)
